classdef MKR_MotorCarrier < handle
    ...
    properties
        u; robotIP; robotPORT; ready; sizeofBuffer; tempSignal; tempData;...
            ...
            RDB_Toggle; RDB_Index; channelToDraw; ultrasonicBuffer; irBuffer; encoderVal;
    ...
    methods
        ...
        function reflectanceSetup(obj)
            %Digital pins 2,3,4,5 are reserved for IR reflectance sensor
        ...
        function val = readEncoder(obj,enc_num)
            sendOverUDP(obj,16,1,enc_num);
            pause(0.05);
            val = str2num(obj.encoderVal);
        end
        ...
        function read(obj, varargin)
            ...
            for i = 1:datagramCount
                ...
                switch(obj.tempSignal(1))
                    ...
                    case "ENC"
                        obj.encoderVal = obj.tempSignal(2);
                end
            end
        end
